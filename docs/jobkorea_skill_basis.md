...
기존 학습자료 DB를 단순히 많이 쓰이는 개발 학습자료 기준으로 고르는 대신, 잡코리아 채용공고에서 반복적으로 보이는 요구역량을 먼저 정리하고 그 역량을 보완할 자료로 80개 DB를 다시 구성했다.
...
- 학습자료는 공식문서를 우선하고, 실습플랫폼, 인기 유튜브, 블로그, 책을 보조로 섞었다.
