...
API key, 네트워크, OpenAI 계정 문제가 생기면 `BAAI/bge-m3`를 로컬 fallback으로 사용한다.
...
- 그래서 최종 구현에서는 `text-embedding-3-small`을 기본값으로 두되, API 문제가 생기면 `BAAI/bge-m3` 로컬 fallback을 사용한다.
...
| `BAAI/bge-m3` | 로컬 embedding retriever | 100개 이상 언어, 8192 token, dense/sparse/ColBERT 검색 설명 가능, API 비용 없음 | 로컬 설치와 모델 다운로드 필요 | API 실패/오프라인 fallback |
...
```text
retriever: text-embedding-3-small 기본, BGE-M3 fallback
...
- API key나 네트워크가 막혀도 로컬에서 임베딩 검색을 계속할 수 있다.
- 한국어와 영어 기술명이 섞인 검색에 적합한 multilingual retrieval 모델이다.
...
| BAAI/bge-m3 | 로컬 fallback 임베딩 | API 실패 시 사용 |
...
단, API key나 네트워크 문제가 생기면 `BAAI/bge-m3` 로컬 fallback을 사용한다.
...
| 항목 | 현재 구현 | 최종 설계 |
|---|---|---|
| 검색 방식 | TF-IDF cosine | text-embedding-3-small dense embedding cosine |
| 청킹 | CSV 1행 | CSV 1행 유지 |
| 벡터 저장 | 없음 | `.npz` cache |
| 벡터 DB | 없음 | numpy 기본, FAISS 선택 |
| LLM 리포트 | 템플릿 | 템플릿 기본, LLM API 선택 |
| 외부 API 의존 | 없음 | OpenAI embedding API 필요 |
| fallback | TF-IDF | BAAI/bge-m3 또는 TF-IDF |
